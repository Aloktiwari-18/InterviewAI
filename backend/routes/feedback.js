const express = require('express');
const { protect } = require('../middleware/auth');
const Interview = require('../models/Interview');
const ATSAnalysis = require('../models/ATSAnalysis');

const router = express.Router();


// ================================
// ✅ GET INTERVIEW FEEDBACK
// ================================
router.get('/interview/:id', protect, async (req, res) => {
  try {
    const interview = await Interview.findOne({
      _id: req.params.id,
      user: req.user._id,
      status: 'completed'
    });

    if (!interview) {
      return res.status(404).json({ error: 'Completed interview not found' });
    }

    res.json({
      scores: interview.scores || {},
      feedback: interview.feedback || {},
      answers: interview.answers || [],
      jobTitle: interview.jobTitle || "",
      duration: interview.duration || 0,
      completedAt: interview.completedAt || null
    });

  } catch (error) {
    console.error("❌ Interview Fetch Error:", error);
    res.status(500).json({ error: "Server error" });
  }
});


// ================================
// ✅ GET DASHBOARD DATA
// ================================
router.get('/dashboard', protect, async (req, res) => {
  try {

    const interviews = await Interview.find({
      user: req.user._id,
      status: 'completed'
    })
      .sort({ createdAt: -1 })
      .limit(5);

    const analyses = await ATSAnalysis.find({
      user: req.user._id
    })
      .sort({ createdAt: -1 })
      .limit(5);

    // 🔥 DEBUG (remove later)
    console.log("📊 ANALYSES:", analyses);

    // ========================
    // ✅ SAFE DATA FORMAT FIX
    // ========================
    const safeAnalyses = analyses.map(a => ({
      _id: a._id,
      fileName: a.fileName,
      createdAt: a.createdAt,

      scores: a.scores || {},

      analysis: {
        matchedKeywords: a.analysis?.matchedKeywords || [],
        missingKeywords: a.analysis?.missingKeywords || [],
        presentSkills: a.analysis?.presentSkills || [],
        missingSkills: a.analysis?.missingSkills || [],
        suggestions: a.analysis?.suggestions || [],
        rewrittenSummary: a.analysis?.rewrittenSummary || "",
        strengths: a.analysis?.strengths || [],
        weaknesses: a.analysis?.weaknesses || []
      }
    }));

    // ========================
    // ✅ AVERAGE SCORE
    // ========================
    let avgScore = 0;

    if (interviews.length > 0) {
      avgScore =
        interviews.reduce((sum, i) => sum + (i.scores?.overall || 0), 0) /
        interviews.length;
    }

    // ========================
    // ✅ RESPONSE
    // ========================
    res.json({
      recentInterviews: interviews,
      recentAnalyses: safeAnalyses,

      stats: {
        totalInterviews: await Interview.countDocuments({ user: req.user._id }),
        completedInterviews: interviews.length,
        averageScore: Math.round(avgScore),
        totalAnalyses: await ATSAnalysis.countDocuments({ user: req.user._id })
      }
    });

  } catch (error) {
    console.error("❌ Dashboard Error:", error);
    res.status(500).json({ error: "Server error" });
  }
});

module.exports = router;